...
requirement of a strong bioinformatics background or coding skills.
...
#### [Homology Modeling Practice A: Quick Modeling with SWISS-MODEL.]{style="color:green"}
...
start](pics/seach.png "SWISS-MODEL modeling"){#fig-swissmodel
...
spatial restraints [@janson2019]. Like Rosetta, Modeller is a command-line tool
that allow a full customization of the modeling, which requires more knowledge
about the process but can be very useful for some types of proteins. However,
both methods have been implemented in some online servers and user-friendly
applications.
...
Bioinformatics](https://swift.cmbi.umcn.nl/teach/B1SEM/B1SEM_8.html).](pics/Rotamers.gif "Backbone and loop modeling"){#fig-rotamers
.figure}
...
that makes sense. How can we know if we can rely in the model? Output models are
...
[FATCAT2](http://fatcat.godziklab.org/) or using molecular visualization apps,
like [ChimeraX](https://www.cgl.ucsf.edu/chimerax/) or
[PyMOL](https://pymol.org/2/) (see [Protein Structure
Display](pdb.html#sec-apps) section).
...
@baker2001.](pics/sali.jpeg "Accuracy and application of protein structure models."){#fig-baker
...
# [Homology Modeling Practice B: Model your favourite protein (YFP).]{style="color:green"}

1.  Use SWISS-MODEL to model a protein with no structure on PDB database. If
    your protein has a structure, you can select an ortholog from other
    organisms (mice, chicken, monkey, dog, fly, yeast...).
2.  Model it also with RosettaCM at Baker's lab site
    (https://robetta.bakerlab.org/). Note that you have to select the ***CM***
    option.

![](images/paste-05D080DB.png){width="518"}

3.  Compare the obtained models and their applications.
